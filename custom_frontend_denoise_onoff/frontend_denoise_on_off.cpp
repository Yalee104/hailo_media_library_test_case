...
media_library_return toggle_frontend_config(MediaLibraryFrontendPtr frontend, bool bDenoise)
{
    ...
    config.denoise_config.enabled = bDenoise;
    std::cout << "Setting Denoise to " << bDenoise << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    CreateNewFrontendAndEncoderStream(media_lib, FRONTEND_CONFIG_DENOISE_ON_FILE, "denoise_on");
    OsdRunTimeChange(media_lib, "Denoise ON");
    denoise_state = 1;
    next_state = 1;
    ...
    while (true) {
        ...
        if (denoise_state != next_state) {

            switch_count++;

            std::string OsdLabel = "Denoise ON";

            if (next_state == 1) {

                std::cout << "Switching to denoise on" << std::endl;
                ...
                std::cout << "Switching to denoise off" << std::endl;

                OsdLabel = "Denoise OFF";
                toggle_frontend_config(media_lib->frontend, false);
            }

            OsdRunTimeChange(media_lib, OsdLabel);
            denoise_state = next_state;    
